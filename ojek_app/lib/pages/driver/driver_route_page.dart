...
class DriverRoutePage extends StatefulWidget {
  ...
  const DriverRoutePage({super.key, required this.order});
...
class _DriverRoutePageState extends State<DriverRoutePage> {
  ...
  double? _distanceKm; // dari OSRM (meters -> km)
  double? _durationMin; // dari OSRM (seconds -> minutes)
  bool _loading = true;
  final BroadcastService _rt = BroadcastService();
  late final ApiService _api;
  LatLng? _driverLatLng;
  bool _followDriver = true;
  bool _completeToggle = false;

  @override
  void initState() {
    super.initState();
    // Ambil ApiService (untuk token) dari RepositoryProvider
    // Gunakan addPostFrame agar context sudah siap
  ...
  Future<void> _subscribeLocation() async {
    try {
      await _rt.subscribeOrderLocation(widget.order.id, (evt) {
        try {
          final raw = evt['data'];
  ...
  Widget build(BuildContext context) {
    ...
    final center = pickup;
    return Scaffold(
      appBar: AppBar(title: const Text('Rute Order')),
      body: Stack(
        children: [
          RouteMap(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Dropoff: ${widget.order.dropoffAddress}',
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis,
                    ),
                    const Divider(),
                    if (_distanceKm != null)
                      Text(
                        'Estimasi jarak: ${_distanceKm!.toStringAsFixed(2)} km',
                      ),
                    if (_durationMin != null)
                      Text(
                        'Estimasi waktu: ${_durationMin!.toStringAsFixed(0)} menit',
                      ),
                    ...
                    Row(
                      children: [
                        ...
                        const Spacer(),
                        Switch(
                          value: _completeToggle,
                          onChanged: (v) async {
                            if (!v) {
                              setState(() => _completeToggle = false);
                              return;
                            }
                            setState(() => _completeToggle = true);
                            try {
                              await _api.completeOrder(widget.order.id);
                              if (!mounted) return;
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Order ditandai selesai')),
                              );
                              Navigator.of(context).pop();
                            } catch (e) {
                              if (!mounted) return;
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('Gagal menyelesaikan order: $e')),
                              );
                              setState(() => _completeToggle = false);
                            }
                          },
      ...
      // Pindahkan aksi selesai ke dalam card (switch)
    );
  }
}
